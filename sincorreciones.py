...
# =============================================================================
# 5. CÓDIGOS ESPECIALES EN VARIABLES + REGLAS LÓGICAS -> NaN
# =============================================================================
...
# =============================================================================
# 10. IMPUTACIÓN DE NAs (por flujo, estadísticos calculados solo en su train)
# =============================================================================
...
valores_imputacion_cli = {col: X_train_cli[col].median() if col in vars_num else X_train_cli[col].mode()[0]
...
valores_imputacion_sub = {col: X_train_sub[col].median() if col in vars_num else X_train_sub[col].mode()[0]
...
# =============================================================================
# 11. ASIMETRÍA, OUTLIERS Y CORRELACIONES (por flujo)
# =============================================================================
# Asimetría (skewness) 
...
# Correlación de Spearman (umbral 0.80) sobre el flujo general
...
# IMC vs perímetro de cintura -> eliminamos cintura en ambos flujos
...
# Versiones para regresión logística (dummies + escaladas), por flujo
...
# Versiones "árbol" (variables en bruto), por flujo
X_train_cli_tree, X_test_cli_tree = X_train_cli.copy(), X_test_cli.copy()
X_train_sub_tree, X_test_sub_tree = X_train_sub.copy(), X_test_sub.copy()

print(f"Dimensiones X_train_cli_rl: {X_train_cli_rl.shape} | X_test_cli_rl: {X_test_cli_rl.shape}")
print(f"Dimensiones X_train_sub_rl: {X_train_sub_rl.shape} | X_test_sub_rl: {X_test_sub_rl.shape}")
...
# =============================================================================
# 12b. FUNCIONES DE EVALUACIÓN (UMBRAL POR CV OUT-OF-FOLD SOBRE EL TRAIN)
# =============================================================================
...
def imprimir_metricas(nombre, m, labels=("Sano", "Diabético")):
    ...
    print(f"                       Pred {neg:<18} Pred {pos}")
    print(f"  Real {neg:<18} {m['tn']:6d}             {m['fp']:6d}")
    print(f"  Real {pos:<18} {m['fn']:6d}             {m['tp']:6d}\n{'='*55}\n")


def mejor_umbral_fbeta_cv(modelo, X_train, y_train, cv, beta=2.0):
    """Umbral F-beta óptimo sobre predicciones out-of-fold (cross_val_predict)
    del TRAIN. Cada muestra se predice con un modelo que no la vio."""
    p_oof = cross_val_predict(modelo, X_train, y_train, cv=cv,
...
def evaluar_con_umbral_cv(nombre, modelo, X_train, y_train, X_test, y_test,
                          cv, labels=("Sano", "Diabético"), beta=2.0):
    """Elige umbral F-beta por CV out-of-fold sobre TRAIN, ajusta el modelo en
    todo el TRAIN y evalúa el TEST con ese umbral fijo (test ajeno a la elección)."""
    umbral, fbeta_cv = mejor_umbral_fbeta_cv(modelo, X_train, y_train, cv, beta=beta)
    print(f"  Umbral F{beta:g} óptimo (CV sobre train): {umbral:.4f}"
          f"  |  F{beta:g} en CV: {fbeta_cv:.4f}")
    modelo.fit(X_train, y_train)
    y_prob = modelo.predict_proba(X_test)[:, 1]
    y_pred = (y_prob >= umbral).astype(int)
    metricas = calcular_metricas(y_test, y_prob, y_pred)
    imprimir_metricas(nombre, metricas, labels=labels)
    return {"y_prob": y_prob, "y_pred": y_pred, "umbral": umbral, **metricas}
...
# Estructura común de los dos flujos (sin undersampling, sin holdout de validación)
flujos = [
    {
        "key": "cli", "titulo": "DIABETES compuesto", "labels": ("Sano", "Diabético"),
        "X_tr_rl": X_train_cli_rl, "X_te_rl": X_test_cli_rl,
        "X_tr_tree": X_train_cli_tree, "X_te_tree": X_test_cli_tree,
        "y_train": y_train_cli, "y_test": y_test_cli,
    },
    {
        "key": "sub", "titulo": "Infradiagnóstico",
        "labels": ("Diagnosticado", "Infradiagnosticado"),
        "X_tr_rl": X_train_sub_rl, "X_te_rl": X_test_sub_rl,
        "X_tr_tree": X_train_sub_tree, "X_te_tree": X_test_sub_tree,
        "y_train": y_train_sub, "y_test": y_test_sub,
    },
]
...
# =============================================================================
mod_rl = LogisticRegression(penalty=None, solver="saga", max_iter=5000, random_state=42)

res_rl = {}

for f in flujos:
    modelo_instancia = clone(mod_rl)
    resultado = evaluar_con_umbral_cv(f"RL sin regularización — {f['titulo']}",
                                      modelo_instancia, f["X_tr_rl"], f["y_train"],
                                      f["X_te_rl"], f["y_test"], cv, labels=f["labels"], beta=2)
    resultado["modelo"] = modelo_instancia
    res_rl[f["key"]] = resultado

# Matrices de confusión
fig, axes = plt.subplots(1, 2, figsize=(12, 4))
for ax, f in zip(axes, flujos):
    ConfusionMatrixDisplay.from_predictions(
        f["y_test"], res_rl[f["key"]]["y_pred"],
        display_labels=list(f["labels"]), ax=ax)
    ax.set_title(f"Matriz de confusión — RL base | {f['titulo']}")
plt.tight_layout()
plt.show()
...
for f in flujos:
    ...
    for nombre_config, cfg in configuraciones_rl.items():
        base = LogisticRegression(penalty=cfg["penalty"], solver="saga",
                                  max_iter=5000, random_state=42)
        
        busqueda = GridSearchCV(base, param_grid=cfg["param_grid"][f["key"]],
                                scoring="roc_auc", cv=cv, n_jobs=-1)
        
        busqueda.fit(f["X_tr_rl"], f["y_train"])
        ...
        resultado = evaluar_con_umbral_cv(f"{nombre_config} — {f['titulo']}",
                                          clone(mejor_modelo), f["X_tr_rl"], f["y_train"],
                                          f["X_te_rl"], f["y_test"], cv, labels=f["labels"], beta=2)
...
for f in flujos:
    for nombre_config, resultado in res_rl_variantes[f["key"]].items():
        ...
        graficar_coeficientes(resultado["modelo"], f["X_tr_rl"].columns,
                              f"{nombre_config} — {f['titulo']}", ax=ax)
        plt.tight_layout()
        plt.show()
        print(f"\n=== Top variables — {nombre_config} — {f['titulo']} ===")
        print(tabla_odds_ratios(resultado["modelo"], f["X_tr_rl"].columns))
...
# =============================================================================
for f in flujos:
    print(f"\n=== Variables eliminadas (coef = 0) — {f['titulo']} ===")
    for nombre_config in ["Lasso (L1)", "Elastic Net"]:
        coefs = pd.Series(res_rl_variantes[f["key"]][nombre_config]["modelo"].coef_[0],
                          index=f["X_tr_rl"].columns)
        eliminadas = coefs[coefs == 0].index.tolist()
        print(f"{nombre_config}: {len(eliminadas)} de {len(coefs)} eliminadas")
        if eliminadas:
            print(f"  → {eliminadas}")
...
for f in flujos:
    ...
    arbol_libre.fit(f["X_tr_tree"], f["y_train"])
    auc_train = roc_auc_score(f["y_train"], arbol_libre.predict_proba(f["X_tr_tree"])[:, 1])
    auc_test  = roc_auc_score(f["y_test"],  arbol_libre.predict_proba(f["X_te_tree"])[:, 1])
    print(f"\n--- Árbol sin restricciones — {f['titulo']} ---")
    print(f"  Profundidad : {arbol_libre.get_depth()} | Hojas: {arbol_libre.get_n_leaves()}")
    print(f"  AUC Train: {auc_train:.4f} | AUC Test: {auc_test:.4f}  ← caída = sobreajuste")
...
for f in flujos:
    busqueda = GridSearchCV(DecisionTreeClassifier(random_state=42),
                            param_grid=param_grid_arbol, scoring="roc_auc", cv=cv, n_jobs=-1)
    busqueda.fit(f["X_tr_tree"], f["y_train"])
    print(f"\n--- Mejor árbol — {f['titulo']} ---")
    print(f"  Mejores parámetros : {busqueda.best_params_}")
    print(f"  AUC en CV (Train)  : {busqueda.best_score_:.4f}")
    mejor_arbol = busqueda.best_estimator_
    resultado = evaluar_con_umbral_cv(f"Árbol de Decisión — {f['titulo']}",
                                      clone(mejor_arbol), f["X_tr_tree"], f["y_train"],
                                      f["X_te_tree"], f["y_test"], cv, labels=f["labels"], beta=2)
    resultado["modelo"] = mejor_arbol
    res_arbol[f["key"]]["Árbol de Decisión"] = resultado

for f in flujos:
    arbol_visual = DecisionTreeClassifier(max_depth=3, random_state=42)
    arbol_visual.fit(f["X_tr_tree"], f["y_train"])
    plt.figure(figsize=(18, 8))
    plot_tree(arbol_visual, feature_names=f["X_tr_tree"].columns,
              class_names=list(f["labels"]), filled=True, rounded=True, fontsize=8)
    plt.title(f"Árbol (prof. 3, ilustrativo) — {f['titulo']}")
    plt.show()

for f in flujos:
    fig, ax = plt.subplots(figsize=(7, 6))
    graficar_importancias(res_arbol[f["key"]]["Árbol de Decisión"]["modelo"],
                          f["X_tr_tree"].columns, f"Árbol de Decisión — {f['titulo']}", ax=ax)
    plt.tight_layout()
    plt.show()
...
for f in flujos:
    busqueda_rf = GridSearchCV(RandomForestClassifier(random_state=42),
                               param_grid=grids_rf[f["key"]], scoring="roc_auc", cv=cv, n_jobs=-1)
    busqueda_rf.fit(f["X_tr_tree"], f["y_train"])
    ...
    resultado = evaluar_con_umbral_cv(f"Random Forest — {f['titulo']}",
                                      clone(mejor_rf), f["X_tr_tree"], f["y_train"],
                                      f["X_te_tree"], f["y_test"], cv, labels=f["labels"], beta=2)
    resultado["modelo"] = mejor_rf
    res_rf[f["key"]]["Random Forest"] = resultado
    # OOB válido aquí: el train conserva la prevalencia real (sin undersampling).
    rf_oob = clone(mejor_rf).set_params(oob_score=True, bootstrap=True)
    rf_oob.fit(f["X_tr_tree"], f["y_train"])
...
for f in flujos:
    ...
    graficar_importancias(modelo, f["X_tr_tree"].columns, f"Importancia MDI — {f['titulo']}", ax=axes[0])
    perm = permutation_importance(modelo, f["X_te_tree"], f["y_test"], scoring="roc_auc",
                                  n_repeats=10, random_state=42, n_jobs=-1)
    top_perm = pd.Series(perm.importances_mean, index=f["X_tr_tree"].columns) \
...
# -----------------------------------------------------------------------------
for f in flujos:
    comparacion = {"Árbol de Decisión": res_arbol[f["key"]]["Árbol de Decisión"],
                   "Random Forest":     res_rf[f["key"]]["Random Forest"]}
    plot_roc(comparacion, f["y_test"], f"Árbol vs RF — ROC — {f['titulo']}")
    plot_pr(comparacion, f["y_test"], f"Árbol vs RF — PR — {f['titulo']}")
    print(f"\n=== Resumen — {f['titulo']} ===")
...
for f in flujos:
    busqueda_gb = GridSearchCV(GradientBoostingClassifier(random_state=42),
                               param_grid=grids_gb[f["key"]], scoring="roc_auc", cv=cv, n_jobs=-1)
    
    busqueda_gb.fit(f["X_tr_tree"], f["y_train"])
    ...
    resultado = evaluar_con_umbral_cv(f"Gradient Boosting — {f['titulo']}",
                                      clone(mejor_gb), f["X_tr_tree"], f["y_train"],
                                      f["X_te_tree"], f["y_test"], cv, labels=f["labels"], beta=2)
...
for f in flujos:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    graficar_importancias(modelo, f["X_tr_tree"].columns,
...
for f in flujos:
    ...
    plot_roc(comparacion, f["y_test"], f"Comparativa árboles — ROC — {f['titulo']}")
    plot_pr(comparacion, f["y_test"], f"Comparativa árboles — PR — {f['titulo']}")
    print(f"\n=== Resumen — {f['titulo']} ===")
    print(tabla_resumen(comparacion))


# -----------------------------------------------------------------------------
# Comparativa final 3 familias (figura del cuerpo)
# -----------------------------------------------------------------------------
for f in flujos:
    comparacion = {
        "Reg. logística":    res_rl[f["key"]],
        "Random Forest":     res_rf[f["key"]]["Random Forest"],
        "Gradient Boosting": res_gb[f["key"]]["Gradient Boosting"],
    }
    plot_roc(comparacion, f["y_test"], f"ROC — {f['titulo']}")
    plot_pr(comparacion,  f["y_test"], f"Precision-Recall — {f['titulo']}")
